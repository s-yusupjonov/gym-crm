...
public class StorageInitializer implements BeanPostProcessor {
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (bean instanceof InMemoryStorage storage) {
            ...
            log.info(
                    "Storage initialized: {} trainers, {} trainees, {} trainings",
    ...
    private void loadTrainers(InMemoryStorage s, String file) {
        try (BufferedReader r = openReader(file)) {
            ...
            while ((line = r.readLine()) != null) {
                ...
                t.setTrainerId(Long.parseLong(p[0].trim()));
                t.setUserId(t.getTrainerId());
                t.setFirstName(p[1].trim());
                t.setLastName(p[2].trim());
                t.setUsername(p[3].trim());
                t.setPassword(p[4].trim());
                t.setActive(Boolean.parseBoolean(p[5].trim()));
                t.setSpecialization(p[6].trim());

                s.getTrainerStorage().put(t.getTrainerId(), t);
            }
        } catch (Exception e) {
            log.error("Failed to load trainers from {}", file, e);
        }
    }
    ...
    private void loadTrainees(InMemoryStorage s, String file) {
        try (BufferedReader r = openReader(file)) {
            ...
            while ((line = r.readLine()) != null) {
                ...
                t.setTraineeId(Long.parseLong(p[0].trim()));
                t.setUserId(t.getTraineeId());
                t.setFirstName(p[1].trim());
                t.setLastName(p[2].trim());
                t.setUsername(p[3].trim());
                t.setPassword(p[4].trim());
                t.setActive(Boolean.parseBoolean(p[5].trim()));
                t.setDateOfBirth(LocalDate.parse(p[6].trim()));
                t.setAddress(p[7].trim());

                s.getTraineeStorage().put(t.getTraineeId(), t);
            }
        } catch (Exception e) {
            log.error("Failed to load trainees from {}", file, e);
        }
    }
    ...
    private void loadTrainings(InMemoryStorage s, String file) {
        try (BufferedReader r = openReader(file)) {
            ...
            log.error("Failed to load trainings from {}", file, e);
        }
    }
